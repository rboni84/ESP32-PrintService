...
<tr><td><span class="m get">GET</span><code>/api/scan</code></td><td><code>refresh</code> (opcional) força novo scan</td><td><code>{"status":"scanning"}</code> ou <code>{"status":"done","networks":[{ssid,rssi,ch,secure}]}</code>. Repita a chamada até <code>done</code>. <code>ssid</code> vem em UTF-8 (nome anunciado em Latin-1 é convertido); na conexão o dispositivo usa os bytes exatos do roteador. Com o STA desconectado, a reconexão fica pausada durante a busca.</td></tr>
